...
description: What is the laboratorioFW_ARDUNO library
---

This library provides the mapping between the Arduino Uno board pinout and the activation pin codes of the Embedded Systems and IoT Laboratory board, according to the adapter project for the board. The library also includes examples for using the peripherals available on the board.

[Click here](https://github.com/Franzininho/biblioteca-laboratorioFW_ARDUNO) to access the library repository on GitHub.
...
To use the library, simply include it in your code file with the line `#include<laboratorioFW_ARDUNO.h>`. After that, you can use the board's activation pin codes in your code.
...
In this example, we use two buttons to control the position of a servo motor. The buttons are associated in the code with left and right directions and change the servo angle, making it possible to move it in both directions. The Servo library was used.
...
In this example we read the temperature and humidity sensor and write the data to the OLED display. The `DTH` and `U8glib` libraries are used.
